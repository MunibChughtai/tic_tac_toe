...
class Game_Board:
    ...
    def display_board(self):
        for board_row in self._board:
            ...
            print('  '.join([(board_position.get_player_symbol() if isinstance(board_position, Player) else board_position) for board_position in board_row]))
    ...
    def is_horizontal_match_found(self, coordinates_to_check, player_symbol):
        xposition, yposition = self._extract_x_and_y_coordinates(coordinates_to_check)
        return all([1 if (isinstance(self._board[xposition][yposition_index], Player) and (self._board[xposition][yposition_index]._symbol==player_symbol)) else 0 for yposition_index in range(len(self._board))])

    def is_vertical_match_found(self, coordinates_to_check, player_symbol):
        xposition, yposition = self._extract_x_and_y_coordinates(coordinates_to_check)
        return all([1 if (isinstance(self._board[xposition_index][yposition], Player) and (self._board[xposition_index][yposition]._symbol==player_symbol)) else 0 for xposition_index in range(len(self._board))])

    def is_diagonal1_match_found(self, player_symbol):
        return all ([1 if (isinstance(self._board[xy_position][xy_position], Player) and self._board[xy_position][xy_position]._symbol==player_symbol) else 0  for xy_position in range(3)])

    def is_diagonal2_match_found(self, player_symbol):
        return all ([1 if (isinstance(self._board[(3-1)-xy_position][0+xy_position], Player) and self._board[(3-1)-xy_position][0+xy_position]._symbol==player_symbol) else 0  for xy_position in range(3)])
